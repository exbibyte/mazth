...
impl<T> Index<usize> for Mat2x1<T>
where
    T: Copy + Clone,
{
    type Output = T;
...
impl<T> IndexMut<usize> for Mat2x1<T>
where
    T: Copy + Clone,
{
...
impl<T> Index<usize> for Mat1x2<T>
where
    T: Copy + Clone,
{
    type Output = T;
...
impl<T> IndexMut<usize> for Mat1x2<T>
where
    T: Copy + Clone,
{
    fn index_mut(&mut self, index: usize) -> &mut T {
...
impl<T> Index<usize> for Mat3x1<T>
where
    T: Copy + Clone,
{
    type Output = T;
...
impl<T> IndexMut<usize> for Mat3x1<T>
where
    T: Copy + Clone,
{
    fn index_mut(&mut self, index: usize) -> &mut T {
...
impl<T> Index<usize> for Mat1x3<T>
where
    T: Copy + Clone,
{
    type Output = T;
...
impl<T> IndexMut<usize> for Mat1x3<T>
where
    T: Copy + Clone,
{
    fn index_mut(&mut self, index: usize) -> &mut T {
...
impl<T> Index<usize> for Mat4x1<T>
where
    T: Copy + Clone,
{
    type Output = T;
...
impl<T> IndexMut<usize> for Mat4x1<T>
where
    T: Copy + Clone,
{
    fn index_mut(&mut self, index: usize) -> &mut T {
...
impl<T> Index<usize> for Mat1x4<T>
where
    T: Copy + Clone,
{
    type Output = T;
...
impl<T> IndexMut<usize> for Mat1x4<T>
where
    T: Copy + Clone,
{
    fn index_mut(&mut self, index: usize) -> &mut T {
    ...
    ($v_type: tt) => {
        ...
        impl Mat2x1<$v_type> {
            ...
            pub fn mul_elem(&self, other: &Mat2x1<$v_type>) -> Option<Mat2x1<$v_type>> {
                Some(Mat2x1 {
                    _val: [self._val[0] * other._val[0], self._val[1] * other._val[1]],
                })
            }
            ...
            pub fn magnitude(&self) -> Option<$v_type> {
                Some(((self._val[0] as $v_type).powi(2) + (self._val[1] as $v_type).powi(2)).sqrt())
            }
                ...
                Some(Mat3x1 {
                    _val: [
                        ((self._val[1] as $v_type * other._val[2] as $v_type)
                            - (self._val[2] as $v_type * other._val[1] as $v_type))
                            as $v_type,
                        ((self._val[2] as $v_type * other._val[0] as $v_type)
                            - (self._val[0] as $v_type * other._val[2] as $v_type))
                            as $v_type,
                        ((self._val[0] as $v_type * other._val[1] as $v_type)
                            - (self._val[1] as $v_type * other._val[0] as $v_type))
                            as $v_type,
                    ],
                })
            }
    ...
    ($v_type: tt) => {
        ...
        impl Mat3<$v_type> {
                    ...
                    for y in 0..3 {
                        ...
                        for x in 0..3 {
                            ...
                            m._val[y * 3 + x] = (self._val[y * 3 + 0] * other._val[x + 0]
                                ...
                                + self._val[y * 3 + 2] * other._val[x + 6])
                                as $v_type;
                        }
                    }
                } else {
                    ...
                    for x in 0..3 {
                        ...
                        for y in 0..3 {
                            ...
                            m._val[x * 3 + y] = (self._val[y + 0] * other._val[x * 3 + 0]
                                ...
                                + self._val[y + 6] * other._val[x * 3 + 2])
                                as $v_type;
                        }
                    }
                }
            ...
            pub fn inverse(&self) -> Option<Mat3<$v_type>> {
                ...
                let mut out = *self;
                *out.index_mut(0, 0) =
                    t.index(1, 1) * t.index(2, 2) - t.index(2, 1) * t.index(1, 2) / determinant;
                *out.index_mut(1, 0) =
                    -(t.index(0, 1) * t.index(2, 2) - t.index(2, 1) * t.index(0, 2)) / determinant;
                *out.index_mut(2, 0) =
                    t.index(0, 1) * t.index(1, 2) - t.index(1, 1) * t.index(0, 2) / determinant;
                *out.index_mut(0, 1) =
                    t.index(1, 0) * t.index(2, 2) - t.index(2, 0) * t.index(1, 2) / determinant;
                *out.index_mut(1, 1) =
                    -(t.index(0, 0) * t.index(2, 2) - t.index(2, 0) * t.index(0, 2)) / determinant;
                *out.index_mut(2, 1) =
                    t.index(0, 0) * t.index(1, 2) - t.index(1, 0) * t.index(0, 2) / determinant;
                *out.index_mut(0, 2) =
                    t.index(1, 0) * t.index(2, 1) - t.index(2, 0) * t.index(1, 1) / determinant;
                *out.index_mut(1, 2) =
                    -(t.index(0, 0) * t.index(2, 1) - t.index(2, 0) * t.index(0, 1)) / determinant;
                *out.index_mut(2, 2) =
                    t.index(0, 0) * t.index(1, 1) - t.index(1, 0) * t.index(0, 1) / determinant;
        ...
        impl Mat4<$v_type> {
                    ...
                    for y in 0..4 {
                        ...
                        for x in 0..4 {
                            ...
                            m._val[y * 4 + x] = (self._val[y * 4 + 0] * other._val[x + 0]
                                ...
                                + self._val[y * 4 + 3] * other._val[x + 12])
                                as $v_type;
                        }
                    }
                } else {
                    ...
                    for x in 0..4 {
                        ...
                        for y in 0..4 {
                            ...
                            m._val[x * 4 + y] = (self._val[y + 0] * other._val[x * 4 + 0]
                                ...
                                + self._val[y + 12] * other._val[x * 4 + 3])
                                as $v_type;
                        }
                    }
                }
            ...
            pub fn inverse(&self) -> Option<Mat4<$v_type>> {
                ...
                *inv.index_mut(3, 3) = self.index(0, 0) * self.index(1, 1) * self.index(2, 2)
                ...
                let det = self.index(0, 0) * inv.index(0, 0)
                    + self.index(1, 0) * inv.index(0, 1)
                    + self.index(2, 0) * inv.index(0, 2)
                    + self.index(3, 0) * inv.index(0, 3);
