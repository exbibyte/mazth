...
fn test_mat1x3() {
    ...
    use mat::Mat3;
    {
...

        {
            ...
            assert!(v3
                .is_equal(&Mat1x3 { _val: [20f64; 3] }, 0.0001f64)
                .expect("Mat1x3 is_equal invalid"));
        }
        {
            let v3 = v.div(&v2).expect("div result invalid");
            assert!(v3
                .is_equal(&Mat1x3 { _val: [5f64; 3] }, 0.0001f64)
                .expect("Mat1x3 is_equal invalid"));
        }
        {
            let v3 = v.plus(&v2).expect("plus result invalid");
            assert!(v3
                .is_equal(&Mat1x3 { _val: [12f64; 3] }, 0.0001f64)
                .expect("Mat1x3 is_equal invalid"));
        }
        {
            let v3 = v.minus(&v2).expect("plus result invalid");
            assert!(v3
                .is_equal(&Mat1x3 { _val: [8f64; 3] }, 0.0001f64)
                .expect("Mat1x3 is_equal invalid"));
        }
